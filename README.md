...
在 portfolio.js 文件中将 `displayMediumBlogs` 设置为 true 或 false，以显示获取的 Medium 博客内容，默认值为 true。

## Change and customize every section according to your need.

#### Personalize page content in `/src/portfolio.js` & modify it as per your need. You will also need to modify `index.html` to change the title and metadata to provide accurate SEO for your personal portfolio.
在 `/src/portfolio.js` 文件中个性化页面内容，并根据您的需求进行修改。您还需要修改 `index.html` 文件，以更改标题和元数据，从而为您的个人作品集提供准确的搜索引擎优化。
...
要上传您自己的简历，请直接将一个 PDF 文件上传至“src/containers/greeting/resume”目录，并将该 PDF 文件重命名为“resume.pdf”。

#### Using Emojis

<!-- For adding emoji 😃 into the texts in `Portfolio.js`, use the `emoji()` function and pass the text you need as an argument. This would help in keeping emojis compatible across different browsers and platforms. -->
要在 `Portfolio.js` 中的文本中添加表情符号 😃 ，请使用 `emoji()` 函数，并将您需要的文本作为参数传递进去。这样有助于确保表情符号在不同浏览器和平台之间保持兼容性。
...
您可以选择一个 Lottie 动画，并从诸如 [这个](https://lottiefiles.com/) 之类的网站将其以 json 格式下载下来。在 `src/assets/lottie` 目录下，将您想要修改的 Lottie json 文件替换为相同名称的文件即可。如果您想更改 Lottie 的选项，请前往 `src/components/displayLottie/DisplayLottie.js` 并修改 `defaultOptions` 对象，您可以参考 [lottie-react 文档](https://www.npmjs.com/package/lottie-react) 以获取关于 `defaultOptions` 对象的更多信息。
...
完成设置后，您应该将您的网站发布到网上。
我们强烈建议您仔细阅读[在 GitHub Pages 上部署](https://create-react-app.dev/docs/deployment/#github-pages)这部分关于 React 的文档。

#### Configuring GitHub Actions (Recommended)
<!-- First you should enable, GitHub Actions for the repository you use. -->
首先，您需要为所使用的仓库启用“GitHub  Actions”。
...
GitHub 上的个人资料和存储库信息仅在部署时创建，如果需要更新这些信息，则需要重新部署网站。因此，我们设置了可配置的[定时任务](https://docs.github.com/en/actions/reference/events-that-trigger-workflows#scheduled-events)，它每周都会部署您的网站，这样您在 GitHub 上更新个人资料后，这些资料就会显示在您的作品集上。您还可以通过使用“工作流触发”事件手动触发它，详情请参阅[此指南](https://github.blog/changelog/2020-07-06-github-actions-manual-triggers-with-workflow_dispatch)。

<!-- - When you are done with the configuration, we highly recommend to read through the [GitHub Actions Configuring a workflow](https://docs.github.com/en/actions/configuring-and-managing-workflows/configuring-a-workflow) docs. -->
完成配置后，我们强烈建议您仔细阅读 [GitHub Actions 中“配置工作流”的相关内容](https://docs.github.com/en/actions/configuring-and-managing-workflows/configuring-a-workflow) 。
